                    ...
                    MainActivity.runOnUiThread(new Runnable({
                        run: function() {
                            try {
                                ...
                                var ndg = new Button(MainActivity);
                                ndg.setText("NoDownGlide: " + (nodownglide ? "on" : "off"));
                                ndg.setOnClickListener(new android.view.View.OnClickListener({
                                    onClick: function(viewarg) {
                                    	nodownglide = !nodownglide;
                                        ndg.setText("NoDownGlide: " + (nodownglide ? "on" : "off"));
                                        if (nodownglide == true) {
                                        	nodownglide_yPos = Player.getY();
                                            clientMessage(client + "§NoDownGlide is on");
                                        }
                                        if (nodownglide == false) {
                                            clientMessage(client + "§NoDownGlide is off");
                                        }
                                    }
                                }));
                                cheatLayout.addView(ndg);
...
function modTick() {
    ...
    if (nodownglide){
    	Entity.setVelY(Player.getEntity(), -0.000000000001);
		Entity.setPosition(Player.getEntity(), Player.get(), nodownglide_yPos, getPlayerZ());
    }
    //jeff
}
